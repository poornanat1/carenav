...
def _is_educational_question(question: str) -> bool:
    ...
    return low.startswith(educational_starts)
...
def _guardrail_analysis(question: str, summary: MemberContext) -> QueryAnalysis | None:
    low = question.lower()
    ...
    if profile_mentioned and condition_topic:
        return QueryAnalysis(
            scope="profile",
    ...
    if profile_mentioned and (broad_have_question or any(term in low for term in condition_terms)):
...
def analyze_member_query(
    ...
    guardrail = _guardrail_analysis(question, summary)
    if guardrail:
        return guardrail
    if gateway is None:
