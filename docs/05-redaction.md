...
Implements spec §4.3. Lives in `carenav/redaction/`. It is the security boundary of the
design and one of the two hard CI gates.

## Position

The redaction layer sits between user input plus tool outputs and the model prompt. The
model, at any tier, only ever sees tokenized text.
...
| **Fine-tuned model** | Free-text PHI the *user* typed ("my name is Jordan and my DOB is…"), third-party names, provider mentions, reformatted DOBs | Fireworks supervised fine-tune returning copied entity values; offsets resolved locally |
...
The eval harness asserts that **zero unredacted PHI patterns ever appear in captured
model inputs** ([09-eval.md](09-eval.md)). This is a hard CI gate: PII-leakage rate must
be 0. A regression blocks merge regardless of any other metric.

For this to work, the model gateway ([06](06-model-tiering.md)) must capture every
outbound prompt so the eval can scan it. Capture is on the cold path and stores redacted
text only.
